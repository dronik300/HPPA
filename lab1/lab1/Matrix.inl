...
template<typename T>
Matrix<T>::Matrix(int lines, int columns, int includedLines, int includedColumns) {
	this->lines = lines;
...
template<typename T>
boolean Matrix<T>::equals(Matrix<T>* anotherMatrix) {
...
template<typename T>
void Matrix<T>::fillWithRandomNumbers() {
	for (int i = 0; i < this->lines; i++) {
		for (int j = 0; j < this->columns; j++) {
			for (int k = 0; k < this->includedLines; k++) {
				for (int t = 0; t < this->includedColumns; t++) {
